...
ProgramOptions ArgParser::Parse(int argc, char* argv[]) {
  ...
  Log(DEBUG, "Procesando argumentos de linea de comandos");
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    Log(DEBUG, "Leyendo -size");
    
    if (std::strcmp(argv[i], "-size") == 0) {
      if (i + 1 >= argc) {
        Log(ERROR, "Falta el valor de -size");
    ...
    Log(DEBUG, "Leyendo -ord");

    if (std::strcmp(argv[i], "-ord") == 0) {
      if (i + 1 >= argc) {
        Log(ERROR, "Falta el valor de -ord");
        throw std::invalid_argument("Falta el valor de -ord.");
      }
      std::string method = argv[++i];
      if (method.size() != 1) {
        Log(ERROR, "El codigo de -ord debe ser una sola letra");
    ...
    Log(DEBUG, "Leyendo -init");

    if (std::strcmp(argv[i], "-init") == 0) {
      if (i + 1 >= argc) {
        Log(ERROR, "Modo de inicializacion invalido");
      ...
      if (options.init_mode == "file") {
        if (i + 1 >= argc) {
          Log(ERROR, "Falta el nombre del fichero tras -init file");
        ...
        Log(ERROR, "Modo de inicializacion no valido");
        throw std::invalid_argument("Modo de inicializacion no valido.");
      }
      continue;
    }

    Log(DEBUG, "Leyendo -trace");
    ...
    Log(ERROR, "Argumento desconocido");
